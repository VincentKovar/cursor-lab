...
// Hidden door easter egg — all knobs live in config.js so non-coders can edit them
const CFG = window.FREEMIUM_CONFIG || {};
const DOOR_CODE = String(CFG.SECRET_DOOR_PASSWORD || 'SKID00').trim().toUpperCase();
const DOOR_DESTINATION = String(CFG.SECRET_DOOR_DESTINATION || '').trim();
const DOOR_MESSAGE = CFG.SECRET_DOOR_MESSAGE || '>> SIGNAL DETECTED\n>> END TRANSMISSION';
const DOOR_TRANSPORT_DELAY = Number(CFG.SECONDS_BEFORE_TRANSPORT) || 6;
function rollDoorRevealScore() {
  const min = Number(CFG.DOOR_APPEARS_MIN_POINTS) || 100;
  const max = Math.max(min, Number(CFG.DOOR_APPEARS_MAX_POINTS) || 300);
...
let doorRevealScore = rollDoorRevealScore();
...
const gunBody = new THREE.Mesh(
  new THREE.BoxGeometry(0.08, 0.09, 0.34),
  new THREE.MeshStandardMaterial({ color: 0x2a2150, roughness: 0.35, metalness: 0.5, emissive: 0x6c5eb5, emissiveIntensity: 0.15 })
);
gunGroup.add(gunBody);
const gunTip = new THREE.Mesh(
  new THREE.BoxGeometry(0.045, 0.045, 0.09),
  new THREE.MeshStandardMaterial({ color: 0xf4e04d, emissive: 0xf4e04d, emissiveIntensity: 0.6 })
);
gunTip.position.set(0, 0.015, -0.21);
gunGroup.add(gunTip);
const muzzleLight = new THREE.PointLight(0xffee00, 0, 5);
muzzleLight.position.set(0, 0.02, -0.45);
gunGroup.add(muzzleLight);
const GUN_REST = new THREE.Vector3(0.3, -0.27, -0.75);
gunGroup.position.copy(GUN_REST);
camera.add(gunGroup);
let gunKick = 0;
let walkCycle = 0;

// Explosion / muzzle light flashes fading out over time
let lightFlashes = [];
function flashLight(pos, color = 0xffcc00, intensity = 5, range = 14, life = 0.35) {
  const l = new THREE.PointLight(color, intensity, range);
  l.position.copy(pos);
  scene.add(l);
  lightFlashes.push({ light: l, life, maxLife: life });
}
...
// ─── Colliders (axis-aligned boxes on the XZ plane) ─────────────────────────
// Every solid prop registers here; players, users, and projectiles all test
// against this list. `h` is the top of the box — things flying above it pass.
const colliders = [];
...
function collideXZ(pos, r) {
  for (const c of colliders) {
    ...
    if (d2 < 1e-8) {
      ...
      const px = (c.hw + r) - Math.abs(pos.x - c.x);
      const pz = (c.hd + r) - Math.abs(pos.z - c.z);
      if (px < pz) pos.x += Math.sign(pos.x - c.x || 1) * px;
      else pos.z += Math.sign(pos.z - c.z || 1) * pz;
    } else {
...
function hitsSolid(pos, r) {
  if (Math.abs(pos.x) > ARENA - 0.35 || Math.abs(pos.z) > ARENA - 0.35) return true; // walls
  for (const c of colliders) {
    if (pos.y > c.h) continue; // flies over low cover
    const nx = Math.max(c.x - c.hw, Math.min(pos.x, c.x + c.hw));
    const nz = Math.max(c.z - c.hd, Math.min(pos.z, c.z + c.hd));
    const dx = pos.x - nx, dz = pos.z - nz;
...
[[-8, -8], [8, -8], [-8, 8], [8, 8]].forEach(([x, z]) => {
  ...
  p.castShadow = true;
  scene.add(p);
  colliders.push({ x, z, hw: 0.6, hd: 0.6, h: WALL_H });
});
...
OBSTACLE_POSITIONS.forEach(([x, z], i) => {
  ...
  scene.add(box);
  colliders.push({ x, z, hw: 0.8, hd: 0.8, h: OBSTACLE_H });
});
...
function createOrb() {
  ...
  const mesh = new THREE.Mesh(geo, mat);
  // Pick a spot clear of pillars/cover so the orb is always reachable
  for (let tries = 0; tries < 10; tries++) {
    const angle = Math.random() * Math.PI * 2;
    const dist = Math.random() * bounds * 0.8;
    mesh.position.set(Math.cos(angle) * dist, 1.3, Math.sin(angle) * dist);
    if (!hitsSolid(mesh.position, 1.2)) break;
  }
...
function triggerChaseMode() {
  ...
  SFX.chase();
  document.getElementById('chaseVignette').classList.add('visible');
...
function endChaseMode() {
  ...
  document.getElementById('chaseVignette').classList.remove('visible');
}

// ─── Sound effects (WebAudio synth — no audio files needed) ──────────────────
let audioCtx = null;
function ac() {
  if (!audioCtx) {
...
function tone({ freq = 440, endFreq = null, type = 'square', dur = 0.15, vol = 0.2, delay = 0 }) {
  const ctx = ac();
  if (!ctx) return;
...
function noiseBurst({ dur = 0.3, vol = 0.3, filterFreq = 1200, endFilter = 100, delay = 0 }) {
  const ctx = ac();
  if (!ctx) return;
  const t0 = ctx.currentTime + delay;
  ...
  filter.frequency.exponentialRampToValueAtTime(Math.max(endFilter, 20), t0 + dur);
  const g = ctx.createGain();
...
const SFX = {
  shoot() { tone({ freq: 920, endFreq: 190, type: 'square', dur: 0.09, vol: 0.12 }); },
  grenadeThrow() { noiseBurst({ dur: 0.18, vol: 0.15, filterFreq: 900, endFilter: 300 }); tone({ freq: 300, endFreq: 500, type: 'sine', dur: 0.18, vol: 0.1 }); },
  explosion() {
    noiseBurst({ dur: 0.55, vol: 0.45, filterFreq: 2500, endFilter: 60 });
    tone({ freq: 110, endFreq: 30, type: 'sine', dur: 0.5, vol: 0.4 });
    // microtransaction shrapnel: a spray of little coin pings
    for (let i = 0; i < 5; i++) tone({ freq: 1300 + Math.random() * 900, type: 'sine', dur: 0.07, vol: 0.08, delay: 0.08 + i * 0.05 });
  },
  trap() { tone({ freq: 160, endFreq: 90, type: 'sawtooth', dur: 0.35, vol: 0.14 }); tone({ freq: 640, type: 'sine', dur: 0.1, vol: 0.08, delay: 0.05 }); },
  hit() { noiseBurst({ dur: 0.06, vol: 0.12, filterFreq: 3000, endFilter: 800 }); },
  kill() { tone({ freq: 700, endFreq: 120, type: 'square', dur: 0.22, vol: 0.14 }); tone({ freq: 1568, type: 'sine', dur: 0.09, vol: 0.1, delay: 0.1 }); tone({ freq: 2093, type: 'sine', dur: 0.12, vol: 0.1, delay: 0.18 }); },
  playerHit() { tone({ freq: 130, endFreq: 60, type: 'sawtooth', dur: 0.18, vol: 0.2 }); },
  playerDeath() { tone({ freq: 400, endFreq: 40, type: 'sawtooth', dur: 1.1, vol: 0.25 }); noiseBurst({ dur: 0.8, vol: 0.2, filterFreq: 1000, endFilter: 40 }); },
  chase() { for (let i = 0; i < 4; i++) tone({ freq: i % 2 ? 880 : 660, type: 'square', dur: 0.12, vol: 0.13, delay: i * 0.13 }); },
  ricochet() { tone({ freq: 2200, endFreq: 400, type: 'triangle', dur: 0.08, vol: 0.07 }); },
  uiBlip() { tone({ freq: 520, type: 'sine', dur: 0.08, vol: 0.1 }); tone({ freq: 780, type: 'sine', dur: 0.08, vol: 0.1, delay: 0.09 }); },
  door() { [220, 277, 330, 440].forEach((f, i) => tone({ freq: f, type: 'sine', dur: 1.2, vol: 0.09, delay: i * 0.18 })); },
};
...
function spawnExplosion(pos) {
  SFX.explosion();
  flashLight(pos.clone().add(new THREE.Vector3(0, 0.8, 0)));
  for (let i = 0; i < 12; i++) {
    const c = document.createElement('canvas');
...
let playerHitSfxCooldown = 0;
function damagePlayer(amount) {
  player.health -= amount;
  if (playerHitSfxCooldown <= 0) { SFX.playerHit(); playerHitSfxCooldown = 0.35; }
...
function restartSession() {
  ...
  doorRevealed = false;
  doorRevealScore = rollDoorRevealScore();
...
function updateDoor(dt) {
  if (!doorRevealed) {
    if (player.score >= doorRevealScore) doorRevealed = true;
...
function submitDoorCode() {
  ...
  if (val === DOOR_CODE) {
    SFX.door();
    const textEl = document.getElementById('backdoorText');
    textEl.textContent = DOOR_MESSAGE;
    const btn = document.getElementById('closeSignalBtn');
    if (DOOR_DESTINATION) {
      // Transported out of the game by the resistance — countdown, then leave.
      btn.textContent = 'ESCAPE NOW';
      textEl.textContent += `\n\n>> EXTRACTION IN ${DOOR_TRANSPORT_DELAY} SECONDS...`;
      transportTimer = setTimeout(() => { window.location.href = DOOR_DESTINATION; }, DOOR_TRANSPORT_DELAY * 1000);
    } else {
      btn.textContent = 'CLOSE SIGNAL';
    }
...
function closeBackdoorScreen() {
  if (DOOR_DESTINATION) { window.location.href = DOOR_DESTINATION; return; }
  document.getElementById('backdoorScreen').classList.remove('visible');
  closeDoorModal();
}
...
// Desktop FPS controls: click the canvas to lock the mouse; move to aim,
// click to fire, Esc to release. Drag-look stays as the unlocked fallback.
...
canvas.addEventListener('mousedown', e => {
  ...
  if (pointerLocked()) { fireWeapon(); return; }
  if (canHover && canvas.requestPointerLock) {
    canvas.requestPointerLock();
    return;
  }
  lookDrag.active = true; lookDrag.lastX = e.clientX;
});
window.addEventListener('mousemove', e => {
  if (pointerLocked()) {
    if (doorModalOpen) return;
    yaw -= e.movementX * 0.0025;
    pitch = THREE.MathUtils.clamp(pitch - e.movementY * 0.0025, -1.25, 1.25);
    return;
  }
...
document.getElementById('btnShoot').addEventListener('touchstart', e => { e.preventDefault(); if (!doorModalOpen && !gamePaused) fireWeapon(); });
...
function updatePlayer(dt) {
  ...
  // Weapon bob + recoil recovery + muzzle flash decay
  if (moving) walkCycle += dt * 9;
  gunKick = Math.max(0, gunKick - dt * 6);
  gunGroup.position.set(
    GUN_REST.x + Math.sin(walkCycle) * 0.012,
    GUN_REST.y + Math.abs(Math.cos(walkCycle)) * 0.014,
    GUN_REST.z + gunKick * 0.14
  );
  gunGroup.rotation.x = gunKick * 0.18;
...
function updateUsers(dt) {
  users.forEach(u => {
    ...
    collideXZ(u.group.position, 0.45);
  });
...
function updateProjectiles(dt) {
  for (let i = projectiles.length - 1; i >= 0; i--) {
    ...
    // Solid geometry: grenades detonate on contact, ad banners crumple
    if (hitsSolid(p.mesh.position, p.type === 'grenade' ? 0.25 : 0.15)) {
      if (p.type === 'grenade') spawnExplosion(p.mesh.position.clone());
      else SFX.ricochet();
      scene.remove(p.mesh);
      projectiles.splice(i, 1);
      continue;
    ...
    // Orb pickup via projectile — the shot is consumed by the orb
    let hitOrb = false;
...
function updateLightFlashes(dt) {
  for (let i = lightFlashes.length - 1; i >= 0; i--) {
    ...
    f.life -= dt;
    f.light.intensity = 5 * Math.max(0, f.life / f.maxLife);
    if (f.life <= 0) {
      scene.remove(f.light);
      lightFlashes.splice(i, 1);
    }
  }
}
